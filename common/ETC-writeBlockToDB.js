...
module.exports = (config, blockData, flush)=> {
  ...
  if (blockData && blockData.number >= 0) {
    ...
    logger.log('verbose','- block #' + blockData.number.toString() + ' inserted.');
  }
  ...
  if(flush && self.bulkOps.length > 0 || self.bulkOps.length >= config.bulkSize) {
    ...
    Block.collection.insert(bulk, function( err, blocks ){
      if ( typeof err !== 'undefined' && err ) {
        if (err.code == 11000) {
            logger.log('warn','Skip: Duplicate DB key : ' +err);
        }else{
          logger.log('error','Error: Aborted due to error on DB: ' + err);
          process.exit(9);
        }
      }else{
        logger.log('info', blocks.insertedCount + ' blocks successfully written.');
      }
    });
  }
}
